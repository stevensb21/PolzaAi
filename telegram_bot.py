...
1️⃣ Отправьте данные сотрудника в любом формате
2️⃣ Если данных не хватает - бот попросит уточнить
3️⃣ Бот автоматически найдет или создаст сотрудника
4️⃣ История чата очищается после каждого заказа
...
async def handle_message(message: Message):
    ...
    # Проверяем, есть ли история чата для этого пользователя
    chat_history = user_chat_histories.get(user_id, [])
    if chat_history:
